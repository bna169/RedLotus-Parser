...
class SneakyClicker:
    ...
    def setup_ui(self):
        ...
        title_label.place(x=70, y=15, width=160, height=25)
        close_btn = tk.Button(
            self.root,
            text="×",
        ...
        close_btn.place(x=260, y=5, width=25, height=25)
        left_label = tk.Label(
            self.root,
            text="Left click",
            font=("Segoe UI", 10),
        ...
        left_label.place(x=30, y=65)
        
        self.left_key_btn = tk.Button(
            self.root,
            text="none",
            font=("Segoe UI", 9),
            fg="#C8C8C8",
            ...
            bd=1,
            relief="solid",
            command=self.capture_left_key
        )
        self.left_key_btn.place(x=105, y=63, width=60, height=24)
        
        self.left_cps_label = tk.Label(
            self.root,
            text="10 CPS",
        ...
        self.left_bar_canvas = tk.Canvas(
            self.root,
            width=235,
            height=10,
            bg="#232323",
        ...
        self.left_progress = self.left_bar_canvas.create_rectangle(
            0, 0, 78, 10, fill="#78DC78", outline=""
        )
        ...
        right_label.place(x=30, y=135)
        
        self.right_key_btn = tk.Button(
            self.root,
            text="none",
            font=("Segoe UI", 9),
            fg="#C8C8C8",
            ...
            bd=1,
            relief="solid",
            command=self.capture_right_key
        )
        self.right_key_btn.place(x=105, y=133, width=60, height=24)
        
        self.right_cps_label = tk.Label(
            self.root,
            text="10 CPS",
            font=("Segoe UI", 10, "bold"),
            fg="#78DC78",
        ...
        self.right_bar_canvas = tk.Canvas(
            self.root,
            width=235,
            height=10,
            bg="#232323",
        ...
        self.right_progress = self.right_bar_canvas.create_rectangle(
            0, 0, 78, 10, fill="#78DC78", outline=""
        )
        ...
        version_label.place(x=10, y=220)
        
    def on_left_bar_click(self, event):
        self.is_dragging_left = True
        self.update_left_cps(event.x)
        
    def on_left_bar_drag(self, event):
        if self.is_dragging_left:
            self.update_left_cps(event.x)
            
    def on_left_bar_release(self, event):
        self.is_dragging_left = False
        
    def update_left_cps(self, x):
        x = max(0, min(235, x))
        new_cps = max(1, min(30, int((x / 235.0) * 30) + 1))
        self.left_cps = new_cps
        self.left_cps_label.config(text=f"{self.left_cps} CPS")
        new_width = int(235 * (self.left_cps / 30.0))
        self.left_bar_canvas.coords(self.left_progress, 0, 0, new_width, 10)
        
    def on_right_bar_click(self, event):
        self.is_dragging_right = True
        self.update_right_cps(event.x)
        
    def on_right_bar_drag(self, event):
        if self.is_dragging_right:
            self.update_right_cps(event.x)
            
    def on_right_bar_release(self, event):
        self.is_dragging_right = False
        
    def update_right_cps(self, x):
        x = max(0, min(235, x))
        new_cps = max(1, min(30, int((x / 235.0) * 30) + 1))
        self.right_cps = new_cps
        self.right_cps_label.config(text=f"{self.right_cps} CPS")
        new_width = int(235 * (self.right_cps / 30.0))
        self.right_bar_canvas.coords(self.right_progress, 0, 0, new_width, 10)
